...
5. **Configurar Entorno**: Añadir linters, formateadores de código (Ej. Prettier/ESLint), y Playwright como framework de pruebas inicial.
6. **Configurar CI/CD**: Copiar `.github/workflows/ci.yml` y `.github/workflows/deploy.yml` al proyecto. Configurar secrets de Azure (AZURE_CREDENTIALS) si aplica.
7. **Documentar el Contexto**: Crear o actualizar el `manifiesto-proyecto.md` detallando arquitectura de software, perfil de despliegue, variables y decisiones Azure aún no ejecutadas.
8. **Implementar el Núcleo (Core)**: Construir la base con contratos de puerto, salud, configuración, persistencia y migraciones compatibles con la decisión aprobada.
9. **Registrar Cierre Inicial**: Generar un informe de cierre de la fase de inicialización documentando tareas, dependencias base y próximos pasos.
